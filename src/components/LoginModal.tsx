...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/65 modal-backdrop animate-in fade-in duration-150 overflow-y-auto">
      <div
        className={`bg-white rounded-3xl max-w-sm w-full shadow-2xl overflow-hidden border border-zinc-200 my-auto flex flex-col ${
          shake ? 'animate-bounce' : ''
        }`}
      >
        {/* Header */}
        <div className="p-5 text-center bg-zinc-50 border-b border-zinc-100 relative">
          <button
            onClick={onClose}
            className="absolute top-3.5 right-3.5 p-1.5 rounded-full text-zinc-400 hover:text-black hover:bg-zinc-200 transition-colors"
          >
            <X size={16} />
          </button>
          <div className="w-11 h-11 rounded-2xl bg-black text-white flex items-center justify-center mx-auto mb-2 shadow-sm">
            <User size={20} />
          </div>
          <h3 className="text-sm font-bold text-black">Login Staf & Penjaga Lantai</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-3.5">
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 text-xs font-bold bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:border-black"
              />
              <User size={14} className="absolute right-3 top-2.5 text-zinc-400" />
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 text-xs font-bold bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:border-black font-mono"
              />
              <button
                type="button"
            ...
            {errorMsg && (
              <p className="text-[10px] font-semibold text-black flex items-center gap-1 mt-1">
          ...
          <div className="pt-2 flex gap-2">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2.5 bg-zinc-100 hover:bg-zinc-200 text-black text-xs font-bold rounded-xl"
            >
              Batal
            </button>
            <button
              type="submit"
              className="flex-1 py-2.5 bg-black hover:bg-zinc-800 text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 touch-press shadow-xs"
            >
